...
export class dashboardService {
  ...
  public downloadPDF(ruta): any {
    ...
    const url = `${this.baseUrl}/${ruta}`;
    this.http
      .post(url, { location: 'report.pdf' }, { responseType: 'blob' })
      .subscribe(
        (response) => {
  ...
  public invoice(id, ruta): any {
    ...
    const url = `${this.baseUrl}/${ruta}/${id}`;
    this.http
      .post(url, { location: 'invoice.pdf' }, { responseType: 'blob' })
      .subscribe(
        (response) => {
          const blob = new Blob([response], { type: mediaType });
  ...
  public getPDF(id, ruta, pdfName): any {
    const mediaType = 'application/pdf';
    const url = `${this.baseUrl}/${ruta}/${id}`;
    this.http
      .post(url, { location: `${pdfName}.pdf` }, { responseType: 'blob' })
      .subscribe(
        (response) => {
          const blob = new Blob([response], { type: mediaType });
          saveAs(blob, `${pdfName}_` + id + '.pdf');
        },
        (e) => {
          throwError(e);
        }
